...
using ChicStoreManagement.Model.SearchParam;

namespace ChicStoreManagement.DAL
{
    public class BaseRepository<T> : IBaseRepositoryDAL<T> where T : class, new()
    {
        /// <summary>
        /// EF上下文对象
        /// </summary>
        ...
        /// <returns></returns>
        public T FindSingleOrDefault(System.Linq.Expressions.Expression<Func<T, bool>> predicate)
        {
            return FindAll().AsQueryable().Where(predicate).AsQueryable().SingleOrDefault();
        }
        /// <summary>
        /// 查询所有
        /// </summary>
