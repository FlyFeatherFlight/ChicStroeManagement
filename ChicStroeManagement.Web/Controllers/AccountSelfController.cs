...
namespace ChicStoreManagement.WEB.Controllers
{
    public class AccountSelfController : Controller
    {
        ...
        public ActionResult EditAccountInfoView() {
           var model= BuildSelfInfo();
            ViewBag.IsManager = model.是否店长;
            ViewBag.IsDesigner = model.是否设计师;
            ViewBag.IsEmployee = model.是否销售;
            ViewBag.Store = model.店铺;
            ViewBag.Employee = model.姓名;
            ViewBag.EmployeesID = model.ID;
