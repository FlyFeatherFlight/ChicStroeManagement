...
namespace ChicStoreManagement.WEB.Controllers
{
    ...
    public class CustomerAnalyzeController : Controller
    {
        ...
        /// <returns></returns>
        // GET: CustomerAnalyze
        public ActionResult AnalyzeIndex(bool role)
        {
            var em = SetEmployee();
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult RecDayReportView(bool role)
        {
            var em = SetEmployee();
            ViewBag.Store = em.店铺ID;
            ViewBag.Employee = em.姓名;
        ...
        /// <returns></returns>
        public ActionResult RecReportJson(int storeID, int? emID, DateTime? startDate, DateTime? endDate)
        {
            RecReportModel model = new RecReportModel();
            int RecCount = 0;//接待数
            int ExceptedNum = 0;//意向数
            int ConfirmCount = 0;//确认数
            int DealCount = 0;//成交数
            try
            {
                ...
                if (model == null)
                {
                    ...
                    RecCount = model.CustomerInfoModels.Count();
            ...
            return Json(new { success = true, msg = "查询成功！", data = model,recNum=RecCount,exceptedNum=ExceptedNum,confirmNum=ConfirmCount,dealNum=DealCount });
        }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult SalesMonthReportView(bool role)
        {
            var em = SetEmployee();
            ViewBag.Store = em.店铺ID;
            ViewBag.Employee = em.姓名;
        ...
        private RecReportModel BuilRecReportModel(int storeID, int? emID, DateTime? startDate, DateTime? endDate)
        {
            ...
            if (models.Count() > 0)
            {
                ...
                if (endDate != null)
                {
                    models = models.Where(p => p.接待日期 <= endDate);
                }
        ...
        private List<RecAnalyzeViewModel> BuildSalesReport_MonthData(int storeID, int? emID, int MonthNum)
        {
            ...
            if (emID > 0 && storeID > 0)
            {
                var m = recModels.Where(p => p.接待人ID == emID&&p.接待日期.Month==MonthNum);
                if (m.Count() > 0)
                {
                    recModels = m.ToList();
                }
            ...
            var modeldate = recModels.GroupBy(p => p.接待人ID);//不同接待人接待数据按店铺分组
            foreach (var item in modeldate)
            {
                ...
                storeAnalyzeModel.店铺ID = storeID;
                storeAnalyzeModel.销售ID = item.Key;
                storeAnalyzeModel.销售姓名 = storeEmployeesBLL.GetModel(p => p.ID == item.Key).姓名;
                storeAnalyzeModel.意向确认人数 = item.Count(p => p.是否确认 == true);
                storeAnalyzeModel.成交人数 = item.Count(p => p.是否成交 == true);
                storeAnalyzeModel.店铺 = storeBLL.GetModel(p => p.ID == storeID).名称;
